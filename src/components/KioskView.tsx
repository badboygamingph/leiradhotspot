...
interface Props {
  ...
  onRevertVoucher?: (id: string) => Promise<void>;
  isDarkMode: boolean;
...
export function KioskView({ vouchers = [], available, used, onGetVoucher, onRevertVoucher, isDarkMode, onRefresh, isRefreshing, isMaintenanceMode }: Props) {
  ...
  return (
    <div className="relative overflow-hidden w-full h-full">
      ...
      >
        <div className={`w-8 h-8 rounded-full border-[3px] shadow-md flex items-center justify-center ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'
          }`}>
          <div className={`w-4 h-4 rounded-full border-2 border-t-transparent animate-spin ${isDarkMode ? 'border-blue-400' : 'border-blue-500'
            }`}></div>
        </div>
      </div>
        ...
        <div className="max-w-md mx-auto space-y-10">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => scroll('left')}
                  className={`p-1 rounded-full border transition-all ${isDarkMode
                ...
                <div className="flex items-center gap-1.5">
                  {[0, 1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
                      className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${activeCardIndex === i
                          ? 'bg-blue-500 w-3.5'
              ...
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[10px] font-bold font-mono tracking-wider px-2 py-0.5 rounded-md uppercase ${isDarkMode ? 'bg-blue-500/20 text-blue-300' : 'bg-blue-100 text-blue-800'
                      }`}>
                      Collected
                    </span>
                  </div>
              ...
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[10px] font-bold font-mono tracking-wider px-2 py-0.5 rounded-md uppercase ${isDarkMode ? 'bg-slate-500/20 text-slate-300' : 'bg-slate-100 text-slate-800'
                      }`}>
                      Collected
                    </span>
                  </div>
              ...
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[10px] font-bold font-mono tracking-wider px-2 py-0.5 rounded-md uppercase ${isDarkMode ? 'bg-emerald-500/20 text-emerald-300' : 'bg-emerald-100 text-emerald-800'
                      }`}>
                      Collected
                    </span>
                  </div>
              ...
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[10px] font-bold font-mono tracking-wider px-2 py-0.5 rounded-md uppercase ${isDarkMode ? 'bg-purple-500/20 text-purple-300' : 'bg-purple-100 text-purple-800'
                      }`}>
                      All Time
                    </span>
                  </div>
              ...
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[10px] font-bold font-mono tracking-wider px-2 py-0.5 rounded-md uppercase ${isDarkMode ? 'bg-amber-500/20 text-amber-300' : 'bg-amber-100 text-amber-800'
                      }`}>
                      Ready
                    </span>
                  </div>
              ...
              {DURATIONS.map((d, index) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`w-full group relative overflow-hidden p-6 rounded-[2rem] border transition-all active:scale-[0.98] text-left flex items-center justify-between ${isDarkMode
                  ...
                  >
                    <div className="flex items-center gap-6">
                      <div className={`w-14 h-14 flex items-center justify-center rounded-2xl border-2 transition-all group-hover:scale-105 group-hover:rotate-3 ${isDarkMode ? 'bg-slate-950 border-slate-800 text-blue-400' : 'bg-slate-50 border-slate-100 text-blue-600'
                        }`}>
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-[10px] px-2.5 py-1 font-bold uppercase tracking-wider ${stock > 0
                      ...
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isDarkMode ? 'bg-slate-800 group-hover:bg-blue-500 group-hover:text-white' : 'bg-slate-50 group-hover:bg-blue-600 group-hover:text-white'
                        }`}>
          ...
          >
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`py-4 rounded-2xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 border ${isDarkMode
              ...
              >
                ...
                {purchasedCodes.length > 0 && (
                  <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${isDarkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-700'
                    }`}>
              ...
              <button
                ...
                className={`py-4 rounded-2xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 border relative ${isDarkMode
                    ? 'bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800 hover:border-slate-700'
                    : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                  }`}
              >
                ...
                {announcementsCount > 0 && (
                  <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${isDarkMode ? 'bg-amber-500/20 text-amber-400' : 'bg-amber-100 text-amber-700'
                    }`}>
                    {announcementsCount}
                  </span>
                )}
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center mt-2 mb-4">
              <div className={`flex items-center gap-3 px-4 py-2 rounded-full border shadow-sm transition-all hover:scale-105 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
                }`}>
                <img src={creatorImg} alt="Dariel Pagsiat Ganzon" className={`w-8 h-8 rounded-full border-2 object-cover ${isDarkMode ? 'border-slate-800' : 'border-white shadow-sm'
                  }`} />
              ...
              <div className="p-8 text-center space-y-8">
                ...
                <div className="grid gap-3">
                  ...
                  <button
                    ...
                    className={`w-full py-4 rounded-2xl font-black uppercase tracking-[0.2em] text-[11px] transition-all border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-white border-slate-200 text-slate-400'
                      }`}
                  >
                    Cancel
                  </button>
          ...
          >
            ...
            >
              <div className="p-10 text-center space-y-8">
                ...
                <div className={`p-8 rounded-[2rem] border-2 border-dashed transition-colors relative group ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'
                  }`}>
                  <span className="text-5xl font-black text-blue-500 tracking-tighter select-all block">
                    {dispensedVoucher.code}
                  </span>
                  <div className={`absolute -bottom-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-500' : 'bg-white border-slate-200 text-slate-400'
                    }`}>
                    Access Code
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={async () => {
                      if (onRevertVoucher && dispensedVoucher) {
                        setIsReverting(true);
                        try {
                          await onRevertVoucher(dispensedVoucher.id);
                          // Remove from purchased codes
                          const newPurchasedCodes = purchasedCodes.filter(c => c.id !== dispensedVoucher.id);
                          setPurchasedCodes(newPurchasedCodes);
                          try {
                            localStorage.setItem('myPurchasedCodes', JSON.stringify(newPurchasedCodes));
                          } catch (e) { }
                          ...
                          alert(err.message || "Failed to revert voucher");
                        } finally {
                    ...
                    className="w-full bg-rose-500/10 text-rose-600 py-4 rounded-xl font-black uppercase tracking-[0.2em] text-[11px] hover:bg-rose-500 hover:text-white active:scale-95 transition-all disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    disabled={isReverting}
                    className="w-full bg-slate-950 text-white py-4 rounded-xl font-black uppercase tracking-[0.3em] text-[11px] hover:bg-slate-800 active:scale-95 transition-all disabled:opacity-50"
                  >
                    CLOSE
                  </button>
                </div>
            ...
            >
              ...
              <div className="pt-4 flex flex-col items-center gap-4">
                ...
                <button
                  ...
                  className={`w-full max-w-[200px] py-3.5 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-2 active:scale-95 ${isDarkMode
