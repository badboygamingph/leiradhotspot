          ...
          <div className="space-y-4">
            {purchasedCodes.map((code, idx) => (
              <motion.div
                ...
                key={idx}
                className={`relative overflow-hidden rounded-[1.5rem] border transition-all hover:shadow-lg ${isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-blue-500/30' : 'bg-white border-slate-200 hover:border-blue-500/30 shadow-sm'
                  }`}
              >
                ...
                <div className="p-5 sm:p-6">
                  <div className="flex items-center justify-between mb-4">
                    <span className={`text-[10px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-md ${isDarkMode ? 'bg-blue-500/20 text-blue-400 border border-blue-500/20' : 'bg-blue-50 text-blue-700 border border-blue-100'
                      }`}>
                      {code.duration}
                    </span>
                  ...
                  <div className={`px-5 py-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between border-2 border-dashed gap-3 ${isDarkMode ? 'border-slate-800 bg-slate-950/80' : 'border-slate-200 bg-slate-50'
                    }`}>
