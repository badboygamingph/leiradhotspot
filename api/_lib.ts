...
app.get("/api/vouchers", async (req, res) => {
  try {
    ...
    const sortedLocal = [...local].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
...
app.post("/api/vouchers/redeem", async (req, res) => {
  try {
    ...
    try {
      ...
      if (voucher) {
        ...
        if (foundLocal) {
          ...
          foundLocal.redeemedAt = new Date().toISOString();
          saveLocalVouchers(local);
        }
    ...
    localVoucher.redeemedAt = new Date().toISOString();
    saveLocalVouchers(local);
